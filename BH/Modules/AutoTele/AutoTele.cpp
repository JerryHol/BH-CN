...
void AutoTele::OnLoad() {
	...
	settingsTab = new UITab("Òýµ¼Ïß", BH::settingsUI);
	unsigned int Y = 12;
	unsigned int col = 40;
	...
	int keyhook_x = 170;
	new Checkhook(settingsTab, col, (Y += 15),&Toggles["Draw Destination"].state, "×Ü¿ª¹Ø");
	new Keyhook(settingsTab, keyhook_x, (Y + 2), &Toggles["Draw Destination"].toggle, "");
	...
	new Texthook(settingsTab, 250, 12, "ÑÕÉ«(µã»÷¸ü»»)");

	new Colorhook(settingsTab, 250, 27, &Colors[0], "Â·¾¶");

	new Colorhook(settingsTab, 250, 42, &Colors[1], "ÏÂ¸ö³¡¾°");

	new Colorhook(settingsTab, 250, 57, &Colors[2], "¶´Ñ¨»òÈÎÎñ");

	new Colorhook(settingsTab, 250, 72, &Colors[3], "´«ËÍµã");

	new Colorhook(settingsTab, 250, 87, &Colors[4], "ÉÏ¸ö³¡¾°");

	new Colorhook(settingsTab, 250, 102, &Colors[5], "¶´Ñ¨");
}
...
void AutoTele::LoadConfig() {
	...
	BH::config->ReadInt("Path Color", Colors[0], 32);
	BH::config->ReadInt("Next Color", Colors[1], 132);
	BH::config->ReadInt("Other Color", Colors[2], 10);
	BH::config->ReadInt("WP Color", Colors[3],138);
	BH::config->ReadInt("Prev Color", Colors[4],11);
	BH::config->ReadInt("Other Extra Color", Colors[5],225);
}
	...
	if(Toggles["Draw Destination"].state) {
		for(int i = 0;i<5;i++) {
			if(Vectors[i].x && Vectors[i].y) {
				...
				//POINT test1 = *p_D2CLIENT_AutomapPos;  //µØÍ¼µÄÖÐÐÄµãÎ»ÖÃºÃÏñ
				int automapMode = *p_D2CLIENT_AutomapMode;   //10ÊÇ´óµØÍ¼£¬20ÊÇÐ¡µØÍ¼
				if (automapMode == 20 || automapMode == 10) {
					long x1 = MyPos.x;
					long y1 = MyPos.y;
					long x2 = Pos.x;
					long y2 = Pos.y;
					long len = sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1));  //ÇóÖ±Ïß¾àÀë
					//if (automapMode == 20 && len > 200) return;   //Ð¡µØÍ¼ÉÏÖ±Ïß¾àÀë´óÓÚÈËÎï200ÂëµÄ²»ÏÔÊ¾Ãû³Æ
					if (len <= 200) {
						Drawing::Crosshook::Draw(Pos.x, Pos.y, Colors[i + 1]);  //»­200ÒÔÄÚµÄÄ¿±ê
					}
					else {
						long R = 200;  //¾àÀë,Èç¹ûÔÚÐ¡µØÍ¼Ä£Ê½ÏÂ£¬ÏßÖ»»­200³¤°É
						x2 = (R * (x2 - x1)) / len + x1;
						y2 = (R * (y2 - y1)) / len + y1;
					}
					Drawing::Linehook::Draw(MyPos.x, MyPos.y, x2, y2, Colors[i + 1]);
				}
				else {
...
void AutoTele::OnLoop() {
	...
	if (playerArea >= 137) {
		return;  //ÕâÀï¹ýÂËµôPD2µÄÐÂµØÍ¼£¬²»È»»á±¨´í£¬±ÈÈç´óºÚÃ«µÄµØÍ¼Levels.txtÖÐµÄid
	}
